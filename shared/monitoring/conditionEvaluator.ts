import type { ConditionEvaluation, ConditionStatus, EventCondition, MetricCondition, MetricOperator, MonitorSnapshot, ThesisCondition } from "../monitoring";
...
function canonicalize(value: unknown): unknown {
  ...
  if (value && typeof value === "object") return Object.fromEntries(Object.entries(value as Record<string, unknown>)
    .filter(([key, child]) => !ignoredConditionKeys.has(key) && child !== undefined)
...
function makeEvaluation(condition: ThesisCondition, now: string, values: Omit<ConditionEvaluation, "id" | "conditionId" | "conditionVersion" | "evaluatedAt" | "changed" | "previousStatus">, previous?: ConditionEvaluation): ConditionEvaluation {
  return { id: crypto.randomUUID(), conditionId: condition.id, conditionVersion: condition.conditionVersion ?? conditionVersion(condition), evaluatedAt: now, ...values, changed: previous ? previous.status !== values.status : values.status !== "pending", previousStatus: previous?.status };
}
...
function unavailableEvaluation(condition: ThesisCondition, snapshot: MonitorSnapshot, state: "missing" | "stale" | "unavailable", now: string, previous?: ConditionEvaluation, source?: ConditionEvaluation["source"], asOf?: string, actualValue?: number): ConditionEvaluation {
  if (previous) return makeEvaluation(condition, now, { status: previous.status, dataState: state, actualValue, targetValue: condition.kind === "metric" ? condition.target : condition.to, source, asOf, explanation: `等待新数据；保留上次${previous.status === "confirmed" ? "成立" : "受损"}结论` }, previous);
  const expired = Boolean(condition.deadline && now.slice(0, 10) > condition.deadline);
  return makeEvaluation(condition, now, { status: expired ? "expired" : "pending", dataState: state, actualValue, targetValue: condition.kind === "metric" ? condition.target : condition.to, source, asOf: asOf ?? snapshot.generatedAt, explanation: expired ? "截止日期已过且仍无可判断数据" : "等待可判断数据" });
}
...
function evaluateMetric(condition: MetricCondition, snapshot: MonitorSnapshot, previous: ConditionEvaluation | undefined, now: string): ConditionEvaluation {
  ...
  if (dataState !== "fresh" || metric?.value === undefined) return unavailableEvaluation(condition, snapshot, dataState === "fresh" ? "missing" : dataState, now, previous, metric?.source, metric?.asOf, metric?.value);
  const status = mapStatus(condition.direction, compare(metric.value, condition.operator, condition.target));
  const targetText = Array.isArray(condition.target) ? `${condition.target[0]}–${condition.target[1]}` : String(condition.target);
  return makeEvaluation(condition, now, { status, dataState: "fresh", actualValue: metric.value, targetValue: condition.target, source: metric.source, asOf: metric.asOf, explanation: `${metric.value} ${condition.operator} ${targetText}，${status === "confirmed" ? "条件成立" : "条件受损"}` }, previous);
}
...
function evaluateEvent(condition: EventCondition, snapshot: MonitorSnapshot, previous: ConditionEvaluation | undefined, now: string): ConditionEvaluation {
  ...
  const matched = condition.occurrence === "not-occurred-by-date" ? (matches.length ? false : today > condition.to ? true : undefined) : (matches.length ? true : today > condition.to ? false : undefined);
  if (matched === undefined) return makeEvaluation(condition, now, { status: "pending", dataState: "fresh", targetValue: condition.occurrence === "within-range" ? `${condition.from}–${condition.to}` : condition.to, asOf: snapshot.eventsAsOf, explanation: `事件窗口开放至 ${condition.to}` }, previous);
  const status = mapStatus(condition.direction, matched);
  const event = matches[0];
  return makeEvaluation(condition, now, { status, dataState: "fresh", actualValue: event?.title ?? "未发生", targetValue: condition.occurrence === "within-range" ? `${condition.from}–${condition.to}` : condition.to, source: event?.source, asOf: event?.scheduledAt ?? snapshot.eventsAsOf, explanation: event ? `发现匹配事件：${event.title}` : `截至 ${condition.to} 未发现匹配事件` }, previous);
}

export function evaluateCondition(input: EvaluateConditionInput): ConditionEvaluation {
  return input.condition.kind === "metric" ? evaluateMetric(input.condition, input.snapshot, input.previousDecisive, input.now) : evaluateEvent(input.condition, input.snapshot, input.previousDecisive, input.now);
}
