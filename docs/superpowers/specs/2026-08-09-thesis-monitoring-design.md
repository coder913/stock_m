...
- 事件快照必须覆盖条件的完整有效期：`within-range` 从显式 `from` 开始，其余事件条件从条件 `createdAt` 的日期开始；批量请求取所有条件最早起点和最晚终点，单条起点晚于终点时将起点收敛到终点，禁止生成倒置区间。
...
同一条件在 `reaffirmed` 后先恢复、随后再次受损或过期，属于新的状态迁移，必须重新派生 `review-needed`。判断不仅比较条件键，还比较复核时间之后的 `changed` 迁移记录。

只有每只股票最新投资逻辑版本的未删除条件参与自动评估。旧版本条件保留为只读历史；用户归档当前逻辑后停止自动评估。研究页通过“基于当前条件新建版本”复制语义字段并生成全新的条件 ID，不改写旧条件。
