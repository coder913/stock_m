...
export function ResearchMonitorPanel(props: ResearchMonitorPanelProps) {
  ...
  useEffect(() => {
    ...
    setViews(latest ? repositories.monitorService.getConditionView(latest.id) : []);
    if (latest) props.onThesisSaved(latest.id);
  ...
  const save = async () => {
    ...
    try {
      ...
      await repositories.monitorService.evaluate({ symbols: [props.symbol], now });
  ...
  const refresh = async () => {
    try { await repositories.monitorService.evaluate({ symbols: [props.symbol], now: new Date().toISOString() }); if (thesisId) setViews(repositories.monitorService.getConditionView(thesisId)); setMessage("监控已刷新"); }
  ...
  return <section className="research-monitor-panel" aria-labelledby="thesis-monitor-title">
    ...
    <div className="monitor-actions"><button type="button" onClick={() => void save()}>保存投资逻辑</button><button type="button" disabled={!thesisId} onClick={() => void refresh()}>刷新监控</button></div>
