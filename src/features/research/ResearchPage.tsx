...
export function ResearchPage() {
  ...
  return <article><p>延迟 15 分钟</p><h1>{data.quote.symbol} · {data.quote.name}</h1><p>{data.quote.price} USD · {data.quote.changePercent}%</p><h2>营收与 EPS 趋势</h2><table><tbody>{data.financials.map((item) => <tr key={item.year}><th>{item.year}</th><td>{item.revenue}</td><td>{item.eps}</td></tr>)}</tbody></table><h2>估值区间</h2><p>{data.valuation.low} / {data.valuation.midpoint} / {data.valuation.high}</p><h2>最新证据</h2><ul>{data.evidence.map((item) => <li key={item.date}>{item.date} · {item.text}</li>)}</ul><button type="button" onClick={save}>保存投资逻辑</button><button type="button" disabled={!thesisSaved} onClick={buy}>确认模拟买入</button><p role="status">{message}</p></article>;
}
