...
export function ResearchPage({ marketClient = defaultMarketClient }: { marketClient?: Pick<MarketApiClient, "getCompany" | "getQuotes" | "getFinancials" | "getFilings" | "getNews"> }) {
  ...
  const [news, setNews] = useState<import("../market/apiDomain").CompanyNewsItem[]>([]);
  ...
  useEffect(() => { void marketClient.getNews(symbol).then((result) => setNews(result.data)).catch(() => undefined); }, [marketClient, symbol]);
