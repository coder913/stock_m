...
export function ResearchPeerComparison({ symbol }: { symbol: string }) {
  ...
  useEffect(() => { void mockDiscoveryRepository.getPeers(symbol).then((result) => { setPeers(result.items); setSource(result.source); }).catch(() => undefined); void mockDiscoveryRepository.listStocks().then((result) => setCandidates(result.items)); }, [symbol]);
