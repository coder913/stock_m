...
export function TodayPage({ marketClient = defaultMarketClient, monitorService, monitorAlertRepository, now = currentTime }: { marketClient?: TodayMarketClient; monitorService?: MonitorRunner; monitorAlertRepository?: MonitorAlertRepository; now?: () => string }) {
  ...
  useEffect(() => { let active = true; const time = now(); reloadAlerts(); void service.evaluate({ now: time }).then(() => { if (active) { setMonitorError(""); reloadAlerts(); } }).catch(() => { if (active) setMonitorError("投资逻辑监控暂时不可用"); }); return () => { active = false; }; }, [service, reloadAlerts, now]);
  ...
  return <div className="today-page">
    <button type="button" onClick={() => { void marketClient.refresh({ resource: "quotes", symbols: ["SPY", "QQQ", "DIA", "IWM"] }).then(async () => { await loadQuotes(); try { await service.evaluate({ now: now() }); setMonitorError(""); } catch { setMonitorError("投资逻辑监控暂时不可用"); } reloadAlerts(); }); }}>刷新市场数据</button>
    ...
    <section className="today-review" aria-labelledby="today-review-title"><h2 id="today-review-title">需要复核</h2>{monitorError && <p role="alert">{monitorError}</p>}<ReviewQueue alerts={monitorAlerts} now={now()} onRead={(id) => { alertRepository.markRead(id, now()); reloadAlerts(); }} onSnooze={(id, until) => { alertRepository.snooze(id, until); reloadAlerts(); }} onArchive={(id) => { alertRepository.archive(id, now()); reloadAlerts(); }} /></section>
