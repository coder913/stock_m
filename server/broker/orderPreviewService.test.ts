...
const dependencies = (overrides: Partial<OrderPreviewDependencies> = {}): OrderPreviewDependencies => ({
  ...
  loadPosition: async () => ({ quantity: "2.00000000" }),
...
test("creates a 60-second signed preview with exact decimal notional", async () => {
  ...
  expect(preview).toMatchObject({
    ...
    normalizedOrder: { symbol: "NVDA", quantity: "1.50000000" },
...
describe.each([
  ...
  ["INSUFFICIENT_PAPER_POSITION", dependencies(), { ...base, side: "sell" as const, quantity: "2.00000001" }],
