...
export function evaluatePortfolioAlerts(input: AlertEvaluationInput): AlertCandidate[] {
  ...
  const drawdownSeverity = input.drawdownPercent === undefined ? undefined : severityFor(input.drawdownPercent, 10, 15); if (drawdownSeverity) result.push({ dedupeKey: `portfolio-drawdown:portfolio:${input.naturalPeriod}`, rule: "portfolio-drawdown", severity: drawdownSeverity, message: "组合回撤超限", currentValue: input.drawdownPercent!, threshold: drawdownSeverity === "critical" ? 15 : 10 });
  return result;
}
