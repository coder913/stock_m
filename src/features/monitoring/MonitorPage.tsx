...
export function MonitorPage({ monitorState: injectedState, now = currentTime }: { monitorState?: MonitorStateService; now?: () => string }) {
  ...
  const [taskHealth, setTaskHealth] = useState<MonitorTaskHealthView>();
  ...
  useEffect(() => { void monitorState.getTaskHealth?.().then(setTaskHealth).catch(() => undefined); }, [monitorState]);
  ...
  return <section className="monitor-page"><h1>投资逻辑监控</h1>
    {taskHealth && <MonitorTaskHealth health={taskHealth} />}
