      ...
      {items.map((draft, index) => {
        ...
        return (
          <fieldset className="condition-draft" key={draft.id}>
            ...
            <label>严重程度<select aria-label="严重程度" value={draft.severity} onChange={(event) => update(index, { ...draft, severity: event.target.value as ConditionDraft["severity"] })}><option value="low">低</option><option value="medium">中</option><option value="high">高</option></select></label>
            {draft.kind === "metric" ? <>
              <label>指标<select aria-label="指标" value={draft.metric} onChange={(event) => update(index, { ...draft, metric: event.target.value as MonitorMetric })}>{metricOptions.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
              <label>比较符<select aria-label="比较符" value={draft.operator} onChange={(event) => { const operator = event.target.value as MetricConditionDraft["operator"]; update(index, { ...draft, operator, target: operator === "between" ? [Number.NaN, Number.NaN] : 0 }); }}><option value=">">&gt;</option><option value=">=">&gt;=</option><option value="<">&lt;</option><option value="<=">&lt;=</option><option value="between">区间</option></select></label>
