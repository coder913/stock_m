...
import { RepositoryProvider, type ApplicationRepositories } from "../../app/repositories";
...
function services() {
  ...
  const portfolio = { append: vi.fn(async (input) => ({ ...input, id: "ledger-1" })) };
  return { thesisService, monitorState, portfolio, created };
}
function renderResearch(state: ReturnType<typeof services>) { const repositories = { portfolio: state.portfolio } as unknown as ApplicationRepositories; return render(<RepositoryProvider value={repositories}><MemoryRouter initialEntries={["/stocks/NVDA"]}><Routes><Route path="/stocks/:symbol" element={<ResearchPage marketClient={client as never} thesisService={state.thesisService as never} monitorState={state.monitorState as never} />} /></Routes></MemoryRouter></RepositoryProvider>); }
afterEach(cleanup);
...
test("uses the immutable server thesis id for paper purchase", async () => {
  ...
  expect(state.thesisService.createConditions).toHaveBeenCalledWith(expect.objectContaining({ thesisVersionId: "server-thesis-1" })); expect(state.portfolio.append).toHaveBeenCalledWith(expect.objectContaining({ thesisVersionId: "server-thesis-1" }));
});
