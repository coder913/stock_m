...
import type { LedgerEventType, PortfolioSettings } from "./domain";
...
import { PortfolioSettingsRepository, toNewYorkMarketDate } from "./portfolioSettingsRepository";
...
export function PortfolioPage({ marketClient = defaultMarketClient, monitorService }: { marketClient?: PortfolioMarketClient; monitorService?: PortfolioMonitorService }) {
  const bootstrapLedger = useMemo(() => new PortfolioLedger(localStorage), []);
  const settingsRepository = useMemo(() => new PortfolioSettingsRepository(localStorage, undefined, () => bootstrapLedger.list().map((event) => toNewYorkMarketDate(event.occurredAt)).sort()[0]), [bootstrapLedger]);
  const [settings, setSettings] = useState<PortfolioSettings>(() => settingsRepository.migrate(bootstrapLedger.list()));
  const ledger = useMemo(() => new PortfolioLedger(localStorage, () => settingsRepository.get().initialCash), [settingsRepository]);
  const splitDecisions = useMemo(() => new SplitDecisionRepository(localStorage), []);
  ...
  const events = useMemo(() => ledger.list(), [ledger, version]);
  const ignoredSplitIds = useMemo(() => splitDecisions.list().map((decision) => decision.sourceEventId), [splitDecisions, version]);
  ...
  const performance = usePortfolioPerformance({ enabled: tab === "performance", client: marketClient, settings, events, ignoredSplitIds, range, revision: version, recoveryNotice: settingsRepository.getRecoveryNotice() });
  const performanceModel: PerformanceViewModel = performance.state.status === "ready" ? performance.state.model : { pendingSplits: [], notices: performance.state.status === "error" ? [performance.state.message] : performance.state.status === "loading" ? ["正在加载绩效"] : [], dataState: "unavailable", provenance: { source: "alpaca" } };
  const historyValues = performance.state.status === "ready" ? performance.state.model.result?.points.flatMap((point) => point.totalValue === undefined ? [] : [point.totalValue]) ?? [] : [];
  ...
  const performanceDrawdown = performance.state.status === "ready" ? performance.state.model.result?.summary.currentDrawdown : undefined;
  ...
  const mutate = (notice: string) => { setMessage(notice); setVersion((value) => value + 1); };
  const saveEvent = () => {
    try {
      const occurredAt = new Date().toISOString();
      if (eventType === "buy") ledger.append({ type: "buy", symbol, quantity: Number(quantity), price: Number(price), thesisVersionId: "v1", occurredAt });
      if (eventType === "sell") ledger.append({ type: "sell", symbol, quantity: Number(quantity), price: Number(price), reason, occurredAt });
      if (eventType === "dividend") ledger.append({ type: "dividend", symbol, amount: Number(amount), reason: reason || "现金分红", occurredAt });
      if (eventType === "fee") ledger.append({ type: "fee", amount: Number(amount), reason: reason || "模拟费用", occurredAt });
      if (eventType === "deposit") ledger.append({ type: "deposit", amount: Number(amount), reason, occurredAt });
      if (eventType === "withdrawal") ledger.append({ type: "withdrawal", amount: Number(amount), reason, occurredAt });
      setDialog(false);
      mutate("交易已记录");
    } catch (error) { setMessage(error instanceof Error ? error.message : "保存失败"); }
  };
  const appendSplit = (input: ConfirmedSplitInput) => { ledger.append({ type: "split", symbol: input.symbol, oldRate: input.oldRate, newRate: input.newRate, quantityMultiplier: input.quantityMultiplier, source: input.source, sourceEventId: input.sourceEventId, confirmedAt: new Date().toISOString(), occurredAt: `${input.effectiveDate}T00:00:00Z` }); mutate("拆股已写入账本"); };
  const saveSettings = (input: Omit<PortfolioSettings, "version" | "updatedAt">) => { try { const saved = settingsRepository.save(input); setSettings(saved); setSettingsDialog(false); mutate("组合设置已保存"); } catch (error) { setMessage(error instanceof Error ? error.message : "设置保存失败"); } };
  const saveBenchmark = async (nextSymbol: string) => { const normalized = nextSymbol.trim().toUpperCase(); if (!/^[A-Z0-9.-]+$/.test(normalized)) throw new Error("基准代码格式无效"); const envelope = await marketClient.getBatchBars([normalized], { start: settings.inceptionDate, end: new Date().toISOString().slice(0, 10), adjustment: "all" }); if ((envelope.data.symbols[normalized] ?? []).length < 2) throw new Error("基准没有有效历史日线"); const saved = settingsRepository.save({ ...settings, benchmarkSymbol: normalized, baseCurrency: "USD" }); setSettings(saved); mutate(`基准已切换为 ${normalized}`); };
  const submitReview = () => { const review = new ReviewRepository(localStorage).submit({ week: "2026-W32", snapshot: { asOf: new Date().toISOString(), positions: result.positions, cash: result.cash, totalValue: result.totalValue, cumulativePnl: result.cumulativePnl, drawdownPercent: performanceDrawdown === undefined ? undefined : performanceDrawdown * 100, sectorExposure: result.sectorExposure, quoteSource: quoteProvenance?.source, quoteAsOf: quoteProvenance?.asOf, quoteStale: quoteProvenance?.stale }, events, alerts: alertRepository.list(), judgment: "按计划执行", action: "本周无操作", result: "组合复盘已生成", nextObservations: ["NVDA 财报"] }); mutate(`2026-W32 · 版本 ${review.version}`); };
