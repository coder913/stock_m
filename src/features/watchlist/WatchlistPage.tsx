...
export function WatchlistPage() {
  ...
  return <section className="watchlist-page"><header><h1>自选</h1><p>分组不会改变股票研究、投资逻辑或模拟持仓。</p></header>
    ...
    {groups.length === 0 ? <p role="status">尚无自选分组。</p> : <div className="watchlist-groups">{groups.map((group, index) => <article key={group.id}><h2>{group.name}</h2><p>{group.symbols.length ? group.symbols.join(" · ") : "暂无股票"}</p><div><button type="button" onClick={() => { repository().moveGroup(group.id, index - 1); refresh(); }} disabled={index === 0}>上移</button><button type="button" onClick={() => { repository().moveGroup(group.id, index + 1); refresh(); }} disabled={index === groups.length - 1}>下移</button><button type="button" onClick={() => { repository().removeGroup(group.id); refresh(); }}>删除 {group.name}</button></div></article>)}</div>}
