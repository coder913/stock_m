...
import type { CompanyNewsItem, CompanyProfile, FinancialFact, SecFiling } from "../../src/features/market/apiDomain";
...
export interface NewsProvider { getNews(symbols: string[], from: string, to: string): Promise<ProviderResult<CompanyNewsItem[]>>; }
...
export function registerCompanyRoutes(app: FastifyInstance, dependencies: { gateway: MarketDataGateway; sec: SecCompanyProvider; profile?: CompanyProfileProvider; news?: NewsProvider; refreshRegistry: RefreshRegistry }): void {
  ...
  app.get("/api/companies/:symbol/news", (request) => { const symbol = symbolFrom((request.params as { symbol: string }).symbol); const query = request.query as { from?: string; to?: string }; return dependencies.gateway.readThrough({ key: `news:${symbol}:${query.from ?? ""}:${query.to ?? ""}`, source: "alpaca", ttlMs: 600_000, load: () => dependencies.news?.getNews([symbol], query.from ?? "", query.to ?? "") ?? Promise.reject(new ApiError("PROVIDER_UNAVAILABLE", "新闻暂不可用", 503, true)) }); });
